...
to create a linked folder (first set of inverted commas) that points to your target folder (second set of inverted commas). 

Hint: copy the absolute paths from your Windows Explorer.

If you require admin rights to do that and do not have them, then use */j* instead of */d*, which ironically is potentially more dangerous (at least in our context). 
...
**Warning:** do not copy linked folders from one location to another (e.g. to replicate them from one R project to others), they seem to paste as the target contents and not as linked folders.
